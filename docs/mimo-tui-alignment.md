...
> 基于 2026-06-26 深度源码调研（2026-06-26 更新），覆盖 `packages/helix-tui/` 全部 34 个源文件、`packages/opencode/src/cli/cmd/tui/` 完整目录、以及 `packages/opencode/src/server/` 后端 API 全量路由。
...
两个 TUI 共享的 Helix Core Engine (`packages/opencode/src/`) 提供以下 API：

| API 类别 | 路由数量 | 关键端点 |
|----------|---------|----------|
| **Session** | 20+ | CRUD, fork, abort, share, summarize, revert, resume |
| **Message** | 8 | prompt (同步流式), prompt_async, command, shell, predict |
| **Permission** | 3 | list, reply (once/always/reject) |
| **Question** | 5 | list, reply, reject, never-ask |
| **MCP** | 8 | status, add, connect, disconnect, OAuth flow |
| **Provider** | 4 | list, auth, OAuth authorize/callback |
| **Config** | 3 | get, update, providers |
| **File** | 6 | list, read, status, find text/files/symbols |
| **PTY** | 6 | CRUD + WebSocket connect |
| **Workflow** | 2 | list, resume |
| **Global** | 6 | health, events, config, dispose, upgrade |

**SSE 事件类型**: 60+ 种，涵盖 session/message/part 生命周期、权限、问题、MCP、LSP、VCS、trace、workflow、actor 等。

**核心子系统**: agent, acp, actor, session, tool, bus, memory, mcp, storage, worktree, config, provider, permission, question, lsp, project, file, sync, task, plugin, skill, snapshot, pty, workflow, metrics, observability
...
| Language (i18n) | ✅ `LanguageProvider` (7 语言: en/zh/zht/ja/es/fr/ru) | ❌ 无 | **关键差距** |
...
**总计: MiMo TUI 19 providers, Helix TUI 4 providers。差距 15 个。**
...
| 语音输入 | ✅ VAD + Whisper 转录 | ✅ 浏览器 SpeechRecognition |
| Shell 模式 | ✅ 命令执行 | ❌ 无 |
| 文件附件 | ✅ 拖拽 + 粘贴 | ❌ 无 |
| 图片附件 | ✅ 图片协议 | ❌ 无 |
| 外部编辑器 | ✅ `$EDITOR` 集成 | ❌ 无 |
...
| `LanguageProvider` + i18n | 国际化框架 + 7 种语言 (en/zh/zht/ja/es/fr/ru) | P1 |
...
| 外部编辑器 | `$EDITOR` 集成 | P2 |
| 文件附件 | 拖拽 + 粘贴 | P2 |
| 语音增强 | 从浏览器 API 迁移到 VAD + Whisper | P2 |
...
| 语音 | 保留浏览器 API + 可选 VAD+Whisper | 渐进增强 |
...
*文档更新时间：2026-06-26 (更新)*
*基于：`packages/helix-tui/` 全量源码 (34 文件, 4565 行) + `packages/opencode/src/cli/cmd/tui/` 全量源码 (27086 行) + `packages/opencode/src/server/` 全量 API (58 模块, 100+ 路由, 60+ SSE 事件) 深度调研*
