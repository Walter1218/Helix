...
| **P2 智能** | 动态智能体分解、自我进化飞轮闭环、模式可扩展 | 任务自主分解、动态 Persona 生成、进化数据闭环、模式可插拔 | 70% 完成——核心能力已接入 runLoop（DynamicAgent、DecompositionGate、AgentStats、OpenSpec），进化飞轮自动化待完成 | 🟢 低 |
...
| 功能 | 前端位置 | 后端状态 | 差距说明 |
|------|----------|----------|----------|
| **Mode Registry** 模式注册表 | `packages/app/src/context/mode-registry.tsx`（6 种模式：Ask/Build/Plan/Compose/Loop/Max） | ✅ 已接入 runLoop | `mode-registry.ts` 已注册到 AppLayer，prompt.ts 通过 `modeRegistry.getEvolutionConfig()` 统一获取模式配置，删除了硬编码副本 |
| **Pre-flight** 启动前检查 | `packages/app/src/pages/session/preflight-panel.tsx`（TrustLevel、CheckItem、操作按钮） | ✅ 已接入 runLoop | `preflight.ts` 已注册到 AppLayer，在 agent 解析后、handle.process 前调用，block 时终止循环 |
| **Cardinal** 阻塞等级 | `packages/app/src/pages/session/cardinal-intervention-panel.tsx`（block/pause/stop/warn 四级） | ✅ 已接入 runLoop | `cardinal.ts` 已注册到 AppLayer，handle.process 返回后运行时检查，四级处理（block/pause/stop/warn） |
| **Judge 裁决 UI** | `packages/app/src/pages/session/judge-verdict-card.tsx` | ✅ 已接入 runLoop | JudgeAgent.make().quickReview() 在 Cardinal 检查后调用，检测 test 文件修改时运行启发式审查 |
...
基础设施层（可观测性、沙箱、记忆、Gateway、飞轮） → 后端已实现 ✅
                    ↓
编排层（Mode Registry、Pre-flight、Cardinal、Judge、AlignmentGuard） → 后端已接入 runLoop ✅
                    ↓
智能层（DecompositionGate、DynamicAgent、AgentStats、OpenSpec） → 已接入 runLoop ✅
                    ↓
状态机层（HybridFSM） → Phase B 观察者模式已接入 runLoop ✅
                    ↓
Pipeline 层（PipelineRunner、AutoDevWorkflow） → 公共服务已提取，scheduler 调用验证通过 ✅
                    ↓
UI 外化层（Trace、AlignmentGuard、Judge、Permission） → 后端有数据，前端无展示 ❌
```

**核心结论**：Helix 的**编排层、智能层、状态机层、Pipeline 层均已完成接入**。下一步是 UI 外化（让用户看到这些能力）。
...
| 编号 | 任务 | 详细描述 | 预计工期 | 关键产出 |
|------|------|----------|----------|----------|
| **P1-1** | **HybridFSM 状态机接入** | **两阶段方案**（见下方 §七详细设计）：Phase B 先作为观察者叠加在 runLoop 上，追踪状态转换并提供 healing 建议；验证效果后 Phase A 替换布尔控制流。代码：`src/session/fsm/hybrid-fsm.ts`（已实现，未接入）。 | 3-5 天（Phase B） | FSM 状态追踪 + healing 建议注入 |
| **P1-2** | **可观测性 UI 外化** | TraceReporter、AlignmentGuard、HeuristicFilter 后端已就绪，但前端用户看不到。按 `docs/ide-ui-design.md` v3 实现：§5.8 执行轨迹树、§5.12 Judge 裁判卡片、§5.13 AlignmentGuard 偏移警告、§6.9 容错降级。目标是让用户"看到 Agent 在想什么"。 | 3-5 天 | 可观测性面板，用户可追踪执行轨迹 |
...
## 七、HybridFSM 状态机接入设计（两阶段方案）

### 背景

`src/session/fsm/hybrid-fsm.ts` 实现了完整的状态机（idle → plan → execute → check → heal → distill → reflect → suspend → complete → failed），支持 healing 循环（最多 3 次重试不同策略）和 reflection 逃生舱（Request_Goal_Revision），但从未被接入。

当前 `processor.ts` 的 runLoop 使用 3 个布尔值（`blocked`/`suspended`/`needsOverflowHandling`）控制流程，配合 5 个 gate（taskGate、goalGate、preflight、cardinal、judge）做决策。

### Phase B：观察者叠加（非侵入式）

**目标**：在不替换现有控制流的前提下，让 FSM 追踪状态并提供 healing 建议。

**接入点**：runLoop 每轮迭代

```
runLoop 开始 → 创建 FSM 实例 → FSM.send(START)
  ↓
handle.process → result
  ↓
FSM 事件映射：
  result="continue" + 有工具调用 → EXECUTE_COMPLETE
  result="continue" + 无工具调用 → CHECK_PASS
  result="stop" / error           → CHECK_FAIL
  result="suspend"                → SUSPEND
  ↓
FSM 状态决策（不直接控制流程，只提供建议）：
  FSM.state="healing"  → 注入 synthetic message："换一种 approach"
  FSM.state="failed"   → 允许停止（覆盖 goalGate 的强制重试）
  FSM.state="reflecting" → 触发 goal revision
  ↓
现有 gate 不变：taskGate → goalGate → break/continue
```

**关键设计**：
1. FSM 不控制 `return "break"/"continue"`，只提供额外信号
2. healing 状态通过注入 synthetic message 实现"换策略"，而非强制重试
3. FSM journal 记录每次迭代的状态转换，用于可观测性
4. `blocked`/`suspended` 布尔值保留，与 FSM 状态同步但不依赖

**预计工期**：3-5 天

### Phase A：完整替换（侵入式，Phase B 验证后）

**前提**：Phase B 运行 1-2 周，收集数据证明 healing/reflection 在实际场景中有效。

**目标**：删除布尔控制流，runLoop 完全由 FSM 驱动。

**接入点**：runLoop 变为 FSM 驱动器

```
FSM.idle → START → FSM.planning
  → resolveTools → PLAN_COMPLETE → FSM.executing
    → handle.process → EXECUTE_COMPLETE → FSM.checking
      → cardinal/judge → CHECK_PASS/FAIL → FSM.healing/distilling
      → healing 重试（最多 3 次，每次注入不同策略）
      → reflecting 逃生舱（Request_Goal_Revision）
```

**关键设计**：
1. 删除 `blocked`/`suspended`/`needsOverflowHandling` 三个布尔值
2. 5 个 gate 重新定位到 FSM 的对应状态中
3. healing 循环由 FSM 的 `maxHealAttempts` 控制
4. reflection 由 FSM 的 `maxReflectionAttempts` 控制

**预计工期**：5-7 天（含全面回归测试）

### 风险与回滚

- Phase B 风险低：FSM 出问题直接移除，核心控制流不受影响
- Phase A 风险中高：需要重新验证所有边界（overflow、compaction、subagent、structured output、max mode）
- 回滚策略：Phase A 如出问题，revert 到 Phase B 的观察者模式
...
- 编排层（Mode Registry、Pre-flight、Cardinal、Judge、AlignmentGuard）**已接入 runLoop** ✅
- 状态机层（HybridFSM）**已实现未接入**，计划两阶段接入 ⚠️
