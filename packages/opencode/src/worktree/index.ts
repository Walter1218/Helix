...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const setup = Effect.fnUntraced(function* (info: Info) {
      ...
      // Write a lockfile with the current PID for WorktreeGC
      const lockFile = pathSvc.join(info.directory, ".mimo-lock")
      yield* fs.writeFile(lockFile, Buffer.from(`PID=${process.pid}\n`)).pipe(Effect.ignore)
    })
    ...
    const boot = Effect.fnUntraced(function* (info: Info, startCommand?: string) {
      ...
      yield* bus.publish(TraceNodeEvent, {
        id: `worktree-${info.name}`,
        type: "node_end",
        name: "shadow_worktree_create",
        status: "success",
        metadata: { directory: info.directory, branch: info.branch },
        timestamp: Date.now(),
