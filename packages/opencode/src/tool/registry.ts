...
import { RequestGoalRevisionTool } from "./request-goal-revision"
import { SuspendTaskTool } from "./suspend-task"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const requestGoalRevision = yield* RequestGoalRevisionTool
    const suspendTask = yield* SuspendTaskTool
    const askUserQuestion = yield* AskUserQuestionTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          requestGoalRevision: Tool.init(requestGoalRevision),
          suspendTask: Tool.init(suspendTask),
          askUserQuestion: Tool.init(askUserQuestion),
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      let filtered = (yield* all()).filter((tool) => {
        ...
        if (tool.id === "Request_Goal_Revision") return true
        if (tool.id === "Suspend_Task") return true

        return true // allow remaining tools
      })
