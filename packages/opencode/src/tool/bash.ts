  ...
  Effect.gen(function* () {
    ...
    const collect = Effect.fn("BashTool.collect")(function* (root: Node, cwd: string, ps: boolean, shell: string) {
      ...
      for (const node of commands(root)) {
        ...
        // Dry-Run AST Interceptor
        if (cmd && HIGH_RISK_COMMANDS.has(cmd.toLowerCase())) {
          log.warn("ToolInterceptor blocked high-risk command", { command: cmd })
          ...
          yield* bus.publish(TraceNodeEvent, {
            ...
            status: "success",
            metadata: { command: cmd, reason: "high-risk" },
            timestamp: Date.now(),
          } as any).pipe(Effect.ignore)
          throw new Error(`ToolInterceptor blocked high-risk command: ${cmd}. For safety reasons, this command is not allowed in the autonomous Loop execution.`)
        }
        if (cmd === "rm") {
          const args = pathArgs(command, ps)
          if (args.includes("/") || args.includes("/*") || args.includes("*") || args.includes(".")) {
            log.warn("ToolInterceptor blocked dangerous rm command", { args })
            const bus = yield* Bus.Service
            yield* bus.publish(TraceNodeEvent, {
              ...
              name: "tool_interceptor_block",
              status: "success",
              metadata: { command: "rm", args, reason: "dangerous-rm" },
              timestamp: Date.now(),
            } as any).pipe(Effect.ignore)
            throw new Error(`ToolInterceptor blocked dangerous rm command: rm with root or wildcard arguments is prohibited.`)
          }
        }
