// @ts-nocheck
import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@mimo-ai/plugin/tui"
...
let _pendingRegistrations: any[] = []
let _hostSlots: any = null

export function getHostSlots() { return _hostSlots }
export function setHostSlots(s: any) {
  _hostSlots = s
  for (const p of _pendingRegistrations) {
    if (s?.register) s.register(p)
  }
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    slots: {
      register(plugin: any) {
        if (_hostSlots?.register) {
          return _hostSlots.register(plugin)
        }
        _pendingRegistrations.push(plugin)
        return () => {}
      },
    },
    plugins: {
