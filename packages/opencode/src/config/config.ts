  ...
  memory: Schema.optional(
    Schema.Struct({
      cc_index: Schema.optional(Schema.Boolean).annotate({
        description:
          "Index Claude Code memory (~/.claude/projects/<slug>/memory) and expose under scope='cc'. Default: false.",
      }),
      vector: Schema.optional(
        Schema.Struct({
          enabled: Schema.optional(Schema.Boolean).annotate({
            description: "Enable vector (embedding-based) semantic search on top of BM25 FTS. When enabled, memory write auto-indexes embeddings and search combines BM25 + vector rerank. Requires a local or remote embedding API. Default: false.",
          }),
          api_url: Schema.optional(Schema.String).annotate({
            description: "Embedding API endpoint (OpenAI-compatible /v1/embeddings). Default: http://localhost:1234/v1/embeddings (LM Studio).",
          }),
          model: Schema.optional(Schema.String).annotate({
            description: "Embedding model name. Default: text-embedding-nomic-embed-text-v1.5 (768 dims, multilingual).",
          }),
        }),
      ),
    }),
