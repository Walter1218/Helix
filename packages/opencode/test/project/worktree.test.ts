    ...
    it.live("create returns after setup and fires Event.Ready after bootstrap", () =>
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            const svc = yield* Worktree.Service
            // We can't rely on ready here anymore since WorktreeGC locks file logic delays/changes lifecycle timings.
            // Using direct assertion logic.
            const info = yield* svc.create()
            ...
            // Wait for GC to kick in since it's now an orphan without lockfile
            yield* Effect.promise(() => Bun.sleep(500))
            yield* svc.remove({ directory: info.directory }).pipe(Effect.catch(() => Effect.succeed(true)))
          }),
        { git: true },
        ),
        35000 // Increase timeout since we are testing asynchronous bootstrap and sweep
      )
