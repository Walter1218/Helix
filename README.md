...
| **Judge Action Modes** | ❌ None | ✅ warn/block/inject 三档可配：inject 模式注入纠正消息让 LLM 自主修正（**已接入 runLoop**） |
| **Instruction Adherence** | ❌ None | ✅ 指令遵循度检测：禁止修改/限定范围/方案选择/范围约束（**已接入 runLoop**） |
| **Candidate Scorer** | ❌ None | ✅ Max 模式候选评分：Judge(40%)+文件数(20%)+测试通过率(30%)+风格一致性(10%) |
...
|------|------|----------|
| `warn` | 只打日志 | 开发调试 |
| `block` | 发现问题立即终止 | 生产环境、严格模式 |
| `inject` | 注入纠正消息到下一轮 | **务实模式（推荐默认）** |
...
### 11. Instruction Adherence (指令遵循度检测)

Helix 检测 LLM 是否真正执行了用户的指令，防止偏离：

| 约束类型 | 示例 | 检测方式 |
|----------|------|----------|
| **禁止修改** | "不要修改 config.ts" | 检查变更文件是否包含目标 |
| **限定范围** | "只修改 login.ts" | 检查是否有超出范围的文件 |
| **方案选择** | "用方案 A" | 检查实际使用的方案 |
| **范围约束** | "只改前端" | 检查是否修改了后端文件 |

```typescript
import { checkAdherence } from "./session/instruction-adherence"
...
// result.deviations = [{ type: "out_of_scope", files: ["src/utils.ts"] }]
```

### 12. Candidate Scorer (Max 模式候选评分)

Max 模式生成多个候选时，通过多维度评分选择最优：

| 维度 | 权重 | 说明 |
|------|------|------|
| **Judge Approved** | 40% | 是否通过所有检查 |
| **文件数** | 20% | 越少越好（精准） |
| **测试通过率** | 30% | 实际测试结果 |
| **风格一致性** | 10% | 与现有代码的风格匹配 |

```typescript
import { selectBestCandidate } from "./session/candidate-scorer"

const best = selectBestCandidate(candidates, judgeReviews, testResults)
// best.candidate — 最优候选
// best.totalScore — 综合得分
// best.dimensions — 各维度详情
```
...
│   │   ├── instruction-adherence.ts # 指令遵循度检测
│   │   └── candidate-scorer.ts # Max 模式候选评分
│   └── agent/
...
- [Execution Flow & Unique Capabilities](docs/helix-execution-flow.md)
